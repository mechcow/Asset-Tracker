class ActiveRecord::Base #:nodoc:
  ...
  module TaggingExtensions
    ...
    def _remove_tags outgoing
      ...
      taggings.destroy(outgoing_taggings.map(&:id))
      end
   ...
   # Returns the tags on <tt>self</tt> as a string.
    ...
    def tag_names
      taggable?(true)
      @tag_names || tags.map(&:name).map(&:capitalize).join(', ')
    end
    ...
    def assign_tags
      taggable?(true)
      if @tag_names
        self.tag_list = @tag_names.split(',').collect do |tag|
          tag.gsub(/\s/,'') unless tag.blank?
        end.compact
      end
    end
