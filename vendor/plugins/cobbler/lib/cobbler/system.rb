...
module Cobbler
  ...
  class System < Base
    ...
    def save
      ...
      Base.make_call('modify_system',sysid,'gateway',  gateway,  token) if gateway
      Base.make_call('modify_system',sysid,'hostname',  hostname,  token) if hostname
      ...
      if @interfaces
        ...
        @interfaces.each do |interface|
          ...
          unless values.empty?
            Base.make_call('modify_system',sysid,'modify_interface',values,token)
            count = count + 1
          end
